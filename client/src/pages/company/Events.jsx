...
import { Calendar, Plus, MapPin, Video, Clock, Users, X, Info, Trash2, CheckCircle2, ChevronRight, Upload, Loader2, Pencil } from 'lucide-react'
...
export default function CompanyEvents() {
  ...
  const { data: jobsData } = useQuery({
    queryKey: ['company-jobs-dropdown', companyName],
  ...
  const { data: eventDetails, isLoading: detailsLoading } = useQuery({
    queryKey: ['event-details', selectedEvent?._id],
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    if (editingEvent) {
      updateEventMutation.mutate({ id: editingEvent._id, updates: formData })
    } else {
  ...
  return (
    <AppLayout title="Manage Events" subtitle="Schedule and coordinate hiring events, pre-placement drives, and virtual talks.">
      <div className="space-y-6">
        
        <PageBanner
          title="Manage Events"
          subtitle="Schedule and coordinate campus recruitment visits, aptitude tests, PPTs, and virtual hiring drives."
          badge="Hiring Events"
          ...
          actions={
            <button
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-teal-500 hover:bg-teal-600 text-white text-sm font-bold rounded-xl shadow-[0_4px_12px_rgba(20,184,166,0.25)] hover:-translate-y-0.5 transition-all"
            >
        ...
        {events.length === 0 ? (
          <div className="bg-white rounded-3xl border border-slate-100 p-16 text-center shadow-sm">
            ...
            <h3 className="text-lg font-bold text-slate-700">No scheduled events</h3>
            <p className="text-slate-400 text-sm mt-1 max-w-sm mx-auto">
              Schedule your first recruitment drive event or PPT to connect with matching students.
            </p>
          </div>
        ) : (
          <div className="grid md:grid-cols-2 gap-6">
            {events.map((event) => {
              ...
              return (
                <div
                  key={event._id}
                  className={`bg-white rounded-2xl border p-5 flex flex-col justify-between shadow-sm hover:shadow-md transition-all relative overflow-hidden ${
                    event.status === 'Cancelled' ? 'border-red-100 opacity-70 bg-red-50/5' : 'border-slate-100'
                  }`}
                >
                  <div className="space-y-3">
                    ...
                    {event.imageUrl && !brokenImages[event._id] && (
                      <div 
                        onClick={() => setLightboxMedia({ type: 'image', url: event.imageUrl, title: event.title, company: event.companyName || companyName })}
                    ...
                    {event.videoUrl && (
                      <div 
                        onClick={() => setLightboxMedia({ type: 'video', url: event.videoUrl, title: event.title, company: event.companyName || companyName })}
                      ...
                      >
                        {event.videoUrl.includes('youtube.com') || event.videoUrl.includes('youtu.be') ? (
                          ...
                          <div className="w-full h-full relative flex items-center justify-center bg-slate-900">
                            ...
                            <div className="absolute w-12 h-12 bg-teal-600 rounded-full flex items-center justify-center text-white shadow-md group-hover/media:scale-110 transition-transform">
                    ...
                    {event.audioUrl && (
                      <div 
                        onClick={() => setLightboxMedia({ type: 'audio', url: event.audioUrl, title: event.title, company: event.companyName || companyName })}
                        className="mt-3 p-3 bg-slate-50 border border-slate-100 hover:border-teal-300 hover:bg-teal-50/5 rounded-xl flex items-center gap-3 cursor-pointer group/audio transition-all"
                      >
                        <div 
                          className="w-10 h-10 rounded-xl bg-teal-500/10 group-hover/audio:bg-teal-500/20 border border-teal-500/20 flex items-center justify-center text-teal-650 shrink-0"
                        ...
                        <div className="flex-1 min-w-0 flex items-center justify-between gap-2">
                          <span className="text-xs font-bold text-slate-655 group-hover/audio:text-teal-600 transition-colors truncate">Listen Podcast / Webinar</span>
                          <audio 
                    ...
                    {event.description && <p className="text-xs text-slate-400 font-medium line-clamp-2">{event.description}</p>}
                    ...
                    <div className="flex flex-col gap-1.5 text-xs text-slate-500 font-semibold pt-2">
                      ...
                      {event.relatedJob && (
                        <span className="flex items-center gap-2"><Info className="w-4 h-4 text-slate-400" /> Linked to: <span className="text-teal-600 font-bold">{event.relatedJob.role}</span></span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mt-5 pt-4 border-t border-slate-100 flex items-center justify-between gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 text-xs font-bold text-teal-600 hover:bg-teal-50 px-3 py-1.5 rounded-xl border border-teal-100 transition-colors"
                    >
                      ...
                      {event.status !== 'Cancelled' && !isPast && (
                        <>
                          <button
                            ...
                            className="px-3 py-1.5 text-xs font-bold text-teal-600 hover:bg-teal-50 border border-teal-100 rounded-xl transition-colors flex items-center gap-1"
                          >
                        ...
                        <div key={student._id} className="p-4 rounded-2xl border border-slate-100 bg-slate-50 flex items-start justify-between gap-4">
                          <div className="min-w-0">
                            ...
                            <div className="flex gap-2.5 mt-2 flex-wrap text-[10px] font-bold uppercase text-slate-500">
                              ...
                              <span className="bg-teal-50 text-teal-600 px-2 py-0.5 rounded-lg border border-teal-100">{student.branch}</span>
            ...
            <div className="fixed inset-0 z-50 flex justify-end">
              ...
              >
                <div className="flex justify-between items-center pb-4 border-b border-slate-100">
                  <h2 className="text-lg font-bold text-slate-900">{editingEvent ? 'Edit Recruitment Event' : 'Schedule Recruitment Event'}</h2>
                  <button 
                    onClick={() => {
                      setDrawerOpen(false)
                      setEditingEvent(null)
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto py-4 space-y-4 pr-1">
                  <div>
                    ...
                    <input
                      type="text"
                      required
                      placeholder="e.g. Google Aptitude Test / PPT"
                      value={formData.title}
                      onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                      className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:border-teal-500 focus:ring-1 focus:ring-teal-500/20 outline-none transition-all"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:border-teal-500 outline-none transition-all"
                      >
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      required
                      placeholder={formData.mode === 'Virtual' ? 'e.g. https://teams.microsoft.com/...' : 'e.g. Seminar Hall 3A, Tech Block'}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Link with Campus Job Listing (Optional)</label>
                    <select
                      value={formData.relatedJob}
                      onChange={(e) => setFormData({ ...formData, relatedJob: e.target.value })}
                      className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:border-teal-500 outline-none transition-all text-slate-700"
                    >
                      <option value="">-- No Linked Job --</option>
                      {jobs.map(job => (
                        <option key={job._id} value={job._id}>{job.role} ({job.package} LPA)</option>
                      ))}
                    </select>
                    <p className="text-[10px] text-slate-400 font-semibold mt-2">
                      Linking a job lists the event under the students' "Eligible Drives" tab and coordinates attendance verification automatically.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      rows="3"
                      placeholder="List topics, requirements, or test instructions here..."
                  ...
                  <div className="space-y-3">
                    ...
                    <div
                      ...
                      } ${formData.imageUrl || formData.videoUrl || formData.audioUrl ? 'border-teal-400 bg-teal-50/5' : 'border-slate-200 hover:border-teal-400 hover:bg-slate-50'}`}
                    >
                      <input
                        ref={mediaInputRef}
                        type="file"
                      ...
                      {isUploadingMedia ? (
                        <div className="flex flex-col items-center justify-center py-2">
                          ...
                          <p className="text-sm font-semibold text-teal-600">Uploading media to server...</p>
                        </div>
                      ) : formData.imageUrl ? (
                        ...
                        <div className="relative py-2 flex flex-col items-center">
                          <div className="w-16 h-16 bg-teal-500/10 border border-teal-500/20 rounded-2xl flex items-center justify-center text-teal-500 mb-2">
                  ...
                  <div className="pt-4 flex gap-3">
                    ...
                    <button
                      ...
                      className="flex-1 py-3 bg-teal-500 hover:bg-teal-600 text-white font-bold rounded-xl text-sm shadow-[0_4px_12px_rgba(20,184,166,0.25)] hover:shadow-[0_4px_16px_rgba(20,184,166,0.35)] disabled:opacity-50 transition-all"
                    >
                      {editingEvent 
                ...
                <div className="flex-1 p-6 flex items-center justify-center min-h-[300px] max-h-[70vh] bg-slate-950/20 overflow-y-auto">
                  ...
                  {lightboxMedia.type === 'audio' && (
                    <div className="bg-slate-800/80 border border-slate-700 p-8 rounded-2xl w-full max-w-md text-center space-y-5 shadow-lg">
                      <div className="w-20 h-20 bg-teal-500/10 border border-teal-500/20 rounded-full flex items-center justify-center mx-auto text-teal-500">
