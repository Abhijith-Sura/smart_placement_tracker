...
export default function CompanyPipelineIndex() {
  const navigate = useNavigate()

  const { data, isLoading } = useQuery({
    queryKey: ['company-jobs-list'],
      ...
      <div className="space-y-6">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">Post a job drive first to see its pipeline here.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {jobs.map((job, i) => (
              <motion.div
                ...
                initial="hidden"
                animate="show"
                onClick={() => navigate(`/company/pipeline/${job._id}`)}
              ...
              >
                ...
                <p className="text-sm text-slate-400 mb-4">{job.company || job.companyName}</p>
